...
int main ( int argc, char **argv )
{
  ...
  do
  {
    if(sensedCells == 6113 || sensedCells == 916)  cout << travelledDistance << endl;
    if ( btMode == false )
    {
      long x = target.getX();
      long y = target.getY();
      ...
      if ( act )
      {
        //NOTE: this code must NOT be used; in the ROS implementation the boolean variable has been used to create
        // a separate thread and launch the sweep, Here it's not necessary
      ...
      if ( candidatePosition.size() == 0 )
      {
        ...
        else
        {
          ...
          cout << "Total exploration time: " << travelledDistance / 0.5 << endl;  // robot assumed to move at 0.5m/s
